...
const Admissiontemplate3 = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate that required IDs are present before sending
    // if (!formData.programAppliedFor || !formData.disciplineSpecificCore) {
    //   alert(
    //     "Please select both a Program and a Discipline Specific Core subject.",
    //   );
    //   return;
    // }
    ...
    const payload = {
      ...formData,
      // Ensure these are sent as the IDs stored in your state
      programAppliedFor: formData.programAppliedFor,
      disciplineSpecificCore: formData.disciplineSpecificCore,
      // Map other required fields for appmodel1.js
      email: formData.studentEmail,
      phone: formData.studentMobile,
      aadhaarNumber: formData.aadharNo,
      templateType: "template3",
    };

    try {
      const res = await ep1.post("/api/v2/createApplicationForm", payload);
      if (res.status === 201) {
        setFormData((prev) => ({
          ...prev,
  ...
  useEffect(() => {
    const processPdfDownload = async () => {
      if (isProcessingPdf && formData.applicationNo) {
    ...
    processPdfDownload();
  }, [isProcessingPdf, formData.applicationNo, navigate]);
  useEffect(() => {
    const runPdfGeneration = async () => {
      // Only run if we are in processing mode and have a serial number
      if (isProcessingPdf && formData.applicationNo) {
        try {
          // Small delay to ensure the browser has physically "painted"
          // the new serial number into the HTML
          await new Promise((resolve) => setTimeout(resolve, 500));
    ...
    runPdfGeneration();
  }, [formData.applicationNo, isProcessingPdf, navigate]);
  // Inside Admissiontemplate3 component
  useEffect(() => {
  ...
  useEffect(() => {
    const generateAndNavigate = async () => {
      // Only run if we have a number and the flag is true
      if (isProcessingPdf && formData.applicationNo) {
        try {
          // A tiny delay to ensure the DOM has finished painting the number
          await new Promise((resolve) => setTimeout(resolve, 500));
  ...
  const programOptions = availablePrograms;
  // 1. Logic to find the specific subjects for the selected program
  const currentSubjects = subjectConfigs;
  const selectedProgramName =
    availablePrograms.find((p) => p._id === formData.programAppliedFor)
      ?.program || "";

  const selectedSubjectName =
    subjectConfigs.find((s) => s._id === formData.disciplineSpecificCore)
      ?.subjectName || "";

  // 2. Modified handleChange to reset subjects when program changes
  const handleProgramChange = (e) => {
    setFormData({
      ...formData,
      programAppliedFor: e.target.value,
      disciplineSpecificCore: "",
    });
  };
  // Filter subjectConfigs based on the selected program name
  const filteredSubjects = subjectConfigs.filter(
    (subject) => subject.programName === formData.programAppliedFor,
  );
  return (
        ...
        <form onSubmit={handleSubmit}>
          {step === 1 && (
            <Box>
              ...
              {/* CHANGED FROM RADIO TO DROPDOWN */}
              <TextField
                select
                fullWidth
                ...
                onChange={handleProgramChange} // Keeps your logic to resetagC on change
                required
              >
                {programOptions.map((prog) => (
                  <MenuItem key={prog._id} value={prog.program}>
                    {prog.program}
                  </MenuItem>
                ))}
              </TextField>
              <TextField
                select
                fullWidth
                label="Discipline Specific Core (DSC) *"
                name="disciplineSpecificCore"
                value={formData.disciplineSpecificCore}
                onChange={handleChange}
                sx={{ mt: 3 }}
                required
                disabled={!formData.programAppliedFor}
              >
                {filteredSubjects.length > 0 ? (
                  filteredSubjects.map((subject) => (
                    <MenuItem key={subject._id} value={subject.fullName}>
                      {subject.programName}: {subject.subjectName} (
                      {subject.fullName})
                    </MenuItem>
                  ))
                ) : (
                  <MenuItem disabled>
                    {formData.programAppliedFor
          ...
          >
            <div style={{ width: "70%" }}>
              ...
              >
                ...
                >
                  Application number :{formData.applicationNo || ""}
                </Box>
          ...
          <table style={tableStyle}>
            <tbody>
              ...
              <tr>
                ...
                <td style={cellStyle}>{formData.email}</td>
                <td style={labelStyle}>Student Mobile:</td>
                <td style={cellStyle}>{formData.phone}</td>
              </tr>
              <tr>
              ...
              <tr>
                ...
                <td style={cellStyle}>{formData.aadhaarNumber}</td>
