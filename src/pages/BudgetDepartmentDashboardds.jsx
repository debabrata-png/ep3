...
const BudgetDepartmentDashboardds = () => {
    ...
    const fetchBudgets = async () => {
        try {
            ...
            // FILTER ONLY FOR DEPARTMENT
            const deptItems = items.filter(b => b.department === global1.department);
            setBudgets(deptItems.map(i => ({ ...i, id: i._id })));
        } catch (e) { console.error(e); }
    };
    ...
    return (
        <Box p={3}>
            <Typography variant="h5" gutterBottom>Department Wise Budget Dashboard</Typography>
            ...
            <Button variant="contained" disabled={isDeptFrozen} sx={{ mb: 2 }} onClick={() => { setOpenBudget(true); setEditBudgetId(null); setBudgetForm({ budgetname: '', year: '', department: global1.department || '', budgettype: '', remarks: '' }) }}>
                Create New Budget
            </Button>
...
export default BudgetDepartmentDashboardds;
