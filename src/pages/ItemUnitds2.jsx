...
const ItemUnitds2 = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            if (editId) await ep1.post(`/api/v2/updateitemunitds2?id=${editId}`, payload);
            else await ep1.post('/api/v2/additemunitds2', payload);
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete?')) {
            await ep1.delete(`/api/v2/deleteitemunitds2?id=${id}`);
            fetchData();
        }
    };
