...
const ItemCategoryds2 = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            if (editId) await ep1.post(`/api/v2/updateitemcategoryds2?id=${editId}`, payload);
            else await ep1.post('/api/v2/additemcategoryds2', payload);
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete?')) {
            await ep1.delete(`/api/v2/deleteitemcategoryds2?id=${id}`);
            fetchData();
        }
    };
...
export default ItemCategoryds2;
