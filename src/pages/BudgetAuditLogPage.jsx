...
const BudgetAuditLogPage = () => {
    ...
    const fetchLogs = async () => {
        setLoading(true);
        try {
            const response = await ep1.get(`/api/v2/getbudgetauditlogs?colid=${global1.colid}`);
    ...
    return (
        <Box p={3} sx={{ height: '90vh', bgcolor: '#f8f9fa' }}>
            <Paper elevation={3} sx={{ p: 3, height: '100%', borderRadius: 2 }}>
                <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
                    <Box display="flex" alignItems="center" gap={2}>
                        <HistoryIcon color="primary" sx={{ fontSize: 32 }} />
                        <Typography variant="h5" fontWeight="bold">Budget Audit Logs</Typography>
                    </Box>
                    <Stack direction="row" spacing={1}>
                        <IconButton onClick={() => setRefreshTrigger(p => p + 1)} disabled={loading}>
                            <RefreshIcon />
                        </IconButton>
                    </Stack>
                </Box>
                ...
                <Box sx={{ height: 'calc(100% - 80px)', width: '100%' }}>
                    <DataGrid
                        ...
                        sx={{
                            '& .MuiDataGrid-columnHeaders': { bgcolor: '#f1f3f4' },
                            '& .MuiDataGrid-row:hover': { bgcolor: '#f8f9fa' },
                            border: 'none'
                        }}
                    />
                </Box>
...
export default BudgetAuditLogPage;
