...
const StoreManagerDashboardds2new = () => {
    ...
    const fetchRequests = async () => {
        try {
            ...
            const mapRes = await ep1.get(`/api/v2/getallstoreuserds2?colid=${global1.colid}`);
            const mappings = mapRes.data.data.storeUsers || [];
            // Match against both 'user' and 'userid' for robustness
            const userMappings = mappings.filter(m => m.user === global1.user || m.userid === global1.user); 
            ...
            let filtered = all.filter(r => r.reqstatus !== 'Rejected');
            
            if (statusFilter !== 'All') {
                filtered = filtered.filter(r => r.reqstatus === statusFilter);
            }
            
            setRequests(filtered.map(r => ({ ...r, id: r._id })));
        } catch (error) {
    ...
    const fetchHistory = async () => {
        try {
            ...
            if (userMappings.length > 0) {
                ...
                all = all.filter(r =>
                    allowedStoreIds.includes(r.storeid) || 
                    (r.storename && allowedStoreNames.includes(r.storename.trim().toLowerCase()))
                );
            }
    ...
    return (
        <Box p={3}>
            <Tabs value={tabValue} onChange={(e, v) => setTabValue(v)} sx={{ mb: 2 }}>
                ...
                <Tab label="Create Indent" />
                <Tab label="Indent History" />
                ...
                <Box>
                    <Box sx={{ mb: 2, display: 'flex', gap: 2, alignItems: 'center' }}>
                        ...
                        <Select
                            ...
                            sx={{ minWidth: 200 }}
                        >
                            <MenuItem value="All">All Items (Not Rejected)</MenuItem>
                            <MenuItem value="Pending">Pending</MenuItem>
                            <MenuItem value="Not Approved">Not Approved</MenuItem>
                            <MenuItem value="Indent Created">Indent Created</MenuItem>
                            <MenuItem value="HOI Approved">HOI Approved</MenuItem>
                            <MenuItem value="Purchase Requested">Purchase Requested</MenuItem>
                            <MenuItem value="Approved">Approved</MenuItem>
                        </Select>
                        <Button variant="contained" onClick={fetchRequests} size="small">Refresh</Button>
                    </Box>
