...
const VendorComparisonSheetds2 = () => {
            ...
            const enriched = await Promise.all(filtered.map(async (vi) => {
                if (vi.vendor) {
                    try {
                        ...
                        const vendorRes = await ep1.get(`/api/v2/getvendordsbyid2?id=${vi.vendor}`);
    ...
    return (
        <Box p={3}>
            <Typography variant="h4" gutterBottom>Vendor Comparison Sheet</Typography>
            <Box>
...
export default VendorComparisonSheetds2;
