...
import {
    ...
    FormControl, InputLabel, CircularProgress, Backdrop
} from '@mui/material';
        ...
        reader.onload = async (evt) => {
            try {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    try {
                        const payload = { ...data[i], colid: global1.colid, user: global1.user, name: data[i].itemname || data[i].name };
                        await ep1.post('/api/v2/additemmasterds2', payload);
                        success++;
                    } catch (err) { console.error(err); }
                }
                alert(`Upload complete. Success: ${success}`);
                fetchItems();
            ...
            <Box sx={{ mb: 2, display: 'flex', gap: 2, justifyContent: 'flex-end' }}>
                {tabValue === 0 && (
                    <Button variant="outlined" component="label" startIcon={<UploadIcon />}>
                        Bulk Item Upload <input type="file" hidden accept=".xlsx,.xls" onChange={handleBulkUpload} />
                    </Button>
                )}
