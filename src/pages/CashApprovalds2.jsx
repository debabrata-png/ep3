...
const CashApprovalds2 = () => {
    ...
    const fetchHistory = async () => {
        try {
            const res = await ep1.get(
                `/api/v2/cashapproval2?colid=${global1.colid}&user=${global1.user}`
            );
    ...
    const fetchMasterData = async () => {
        try {
            const [vendorRes, itemRes, unitRes, vendorItemRes] = await Promise.all([
                ...
                ep1.get(`/api/v2/getallitemmasterds2?colid=${global1.colid}`),
                ep1.get(`/api/v2/getallitemunitds2?colid=${global1.colid}`),
    ...
    const handleSubmit = async () => {
        setLoading(true);
        try {
            const response = await ep1.post("/api/v2/cashapproval2", formData);
...
export default CashApprovalds2;
