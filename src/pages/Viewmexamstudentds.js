...
import AddUserModal from './Addmexamstudentds';
import AddUserModalBulk from './Addmexamstudendbulkds';
...
function ViewPage() {
    ...
    const onButtonClick = async (e, row) => {
        e.stopPropagation();
        const response = await ep1.get('/api/v2/deleteexamstudentdsrecord', {
            params: { id: row._id, token, user }
        });
    ...
    const columns = [
        { field: 'studentname', headerName: 'Student Name', width: 180, editable: true, valueFormatter: (params) => params.value || '' },
        { field: 'studentregno', headerName: 'Reg No', width: 130, editable: true, valueFormatter: (params) => params.value || '' },
        { field: 'program', headerName: 'Program', width: 150, editable: true, valueFormatter: (params) => params.value || '' },
        { field: 'programcode', headerName: 'Program Code', width: 130, editable: true, valueFormatter: (params) => params.value || '' },
        { field: 'course', headerName: 'Course', width: 150, editable: true, valueFormatter: (params) => params.value || '' },
        { field: 'coursecode', headerName: 'Course Code', width: 130, editable: true, valueFormatter: (params) => params.value || '' },
    ...
    const fetchViewPage = async () => {
        const response = await ep1.get('/api/v2/getexamstudentdsrecords', { params: { token, colid, user } });
        setRows(response.data.data.classes);
    };

    const getgraphdata = async () => {
        const response = await ep1.get('/api/v2/getexamstudentdscountbyyear', { params: { token, colid, user } });
        setResults(response.data.data.classes);
    };

    const getgraphdatasecond = async () => {
        const response = await ep1.get('/api/v2/getexamstudentdscountbyprogram', { params: { token, colid, user } });
    ...
    const handleExport = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'ExamStudentDS');
        const excelBuffer = XLSX.write(wb, { bookType: 'xlsx', type: 'array' });
        const data = new Blob([excelBuffer], { type: 'application/octet-stream' });
        saveAs(data, 'ExamStudentDS_data.xlsx');
        setOpenExport(false);
    };
    ...
    const handleOpenEdit1 = async (updatedRow) => {
        await ep1.post('/api/v2/updateexamstudentdsrecord', {
            id: updatedRow._id, user: updatedRow.user, token, name: updatedRow.name, colid,
            studentname: updatedRow.studentname, studentregno: updatedRow.studentregno,
            program: updatedRow.program, programcode: updatedRow.programcode,
            course: updatedRow.course, coursecode: updatedRow.coursecode,
    ...
    const handleOpenEdit = (u) => { setSelectedUser(u); setOpenEdit(true); };
    const handleCloseEdit = () => { setOpenEdit(false); setSelectedUser(null); };
    const handleOpenDelete = (u) => { setSelectedUser(u); setOpenDelete(true); };
            ...
            <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
                ...
                <Grid container spacing={3}>
                    <Grid item xs={6}>
                        ...
                        <BarChart
                            xAxis={[{ id: 'barCategories', data: second.map((l) => l._id || ''), scaleType: 'band', colorMap: { type: 'piecewise', thresholds: [], colors: ['#F6C179', '#C27F1D', '#A6B0A3', '#EDDBAC', '#A6DAEE'] } }]}
                    ...
                    <Grid item xs={6}>
                        ...
                        <PieChart
                            colors={['#D1A3B4', '#BBD1A3', '#A3C4D1', '#EDDBAC', '#A6DAEE']}
