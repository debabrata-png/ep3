...
const Purchasemanagementds2 = () => {
  ...
  const fetchPurchases = async (status = null) => {
    setLoading(true);
    try {
      let url = `/api/v2/getallpurchaseds2?colid=${global1.colid}`;
      if (status) {
        url = `/api/v2/getpurchasedsbystatus2?status=${status}&colid=${global1.colid}`;
      }
  ...
  const fetchVendorProducts = async (vendorname) => {
    try {
      const response = await ep1.get(
        `/api/v2/getvendorproductdsbyvendor2?vendorname=${vendorname}&colid=${global1.colid}`
      );
  ...
  const handleAddPurchase = async () => {
    try {
      ...
      await ep1.post("/api/v2/addpurchaseds2", payload);
  ...
  const handleUpdateStatus = async (id, status) => {
    if (window.confirm(`Are you sure you want to mark this purchase as ${status}?`)) {
      try {
        await ep1.post(`/api/v2/updatepurchaseds2?id=${id}`, { status });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this purchase?")) {
      try {
        await ep1.get(`/api/v2/deletepurchaseds2?id=${id}`);
...
export default Purchasemanagementds2;
