...
function ViewPage() {
    ...
    // Calculate total amount when selection changes
    useEffect(() => {
        let total = 0;
        rowSelectionModel.forEach((id) => {
            const row = rows.find((r) => r._id === id);
            if (row && row.amount && row.status !== 'paid') {
    ...
    const onPaySelected = async () => {
        ...
        const isConfirmed = window.confirm(`Are you sure you want to pay ₹${customAmount} for the selected items?`);
        if (!isConfirmed) {
            return;
        }
        ...
        const selectedRows = rows.filter(r => rowSelectionModel.includes(r._id));
        const firstSelected = selectedRows.length > 0 ? selectedRows[0] : null;
        ...
        const paymentData = {
            ...
            regno: global1.regno || "",
            studentEmail: global1.email || global1.user || "",
            studentPhone: global1.phone || global1.mobile || "0000000000",
            originalAmount: customAmount,
            paymentType: "SEMESTER_FEE",
            paymentPurpose: "Bulk Payment for Selected Items",
            academicYear: firstSelected ? firstSelected.academicyear : "2024-25",
            semester: firstSelected ? firstSelected.semester : "",
            course: "",
            department: "",
            programcode: firstSelected ? firstSelected.programcode || firstSelected.feegroup : "",
            admissionyear: "",
            couponCode: "",
            feegroup: firstSelected ? firstSelected.feegroup : "Payment",
            feeitem: "Bulk Payment",
            feecategory: "Payment",
            installment: "",
            frontendCallbackUrl: `${frontendURL}/hdfcpaymentcallbackds`,
            backendReturnUrl: `${backendURL}/api/v2/hdfcpaymentorderds/return`,
            comments: `Bulk Payment of ${customAmount}`,
            notes: `Selected IDs: ${rowSelectionModel.join(', ')}`
        };
    ...
    return (
        <React.Fragment>
            <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
                {/* Multipayment Summary Section */}
                {rowSelectionModel.length > 0 && (
                    <Paper elevation={3} sx={{ p: 2, mb: 2, display: 'flex', alignItems: 'center', justifyContent: 'space-between', bgcolor: '#f5f5f5' }}>
                        <Box>
                            <Typography variant="h6">Bulk Payment</Typography>
                            <Typography variant="body2">Selected {rowSelectionModel.length} items</Typography>
                        </Box>
                ...
                <Grid container spacing={3}>
                    <Grid item xs={12}>
                        <Paper elevation={5} sx={{ p: 2, display: 'flex', flexDirection: 'column', width: '100%' }}>
                            ...
                            />
                            {/* Modals ... */}
                            {/* (existing modals kept but maybe not used much) */}
                            <AddUserModal
                                open={openAdd}
