        ...
        reader.onload = async (evt) => {
            try {
                ...
                for (const row of data) {
                    try {
                        let payload = { ...row, ...commonPayload };
                        // Adjust keys if necessary or ensure excel headers match payload keys
                        // Assuming Excel headers are exactly the keys we need (like storename, etc.)

                        // Specific adjustments if needed
                        if (currentContext === 'store') {
                            payload.name = payload.storename; // Ensure 'name' is present if required by backend logic (though my formLogic adds it, endpoint expects it?)
                            // My previous form logic: payload.name = currentItem.storename. 
                            // So yes, some endpoints might use 'name'.
                            if (!payload.name) payload.name = payload.storename;
                            await ep1.post('/api/v2/addstoremasterds', payload);
                        }
                        else if (currentContext === 'type') {
                            if (!payload.name) payload.name = payload.itemtype;
                            await ep1.post('/api/v2/additemtypeds', payload);
                        }
                        else if (currentContext === 'category') {
                            if (!payload.categoryname) payload.categoryname = payload.name;
                            await ep1.post('/api/v2/additemcategoryds', payload);
                        }
                        else if (currentContext === 'unit') {
                            if (!payload.unitname) payload.unitname = payload.name;
                            await ep1.post('/api/v2/additemunitds', payload);
                        }
                        else if (currentContext === 'item') {
                            if (!payload.name) payload.name = payload.itemname;
                            await ep1.post('/api/v2/additemmasterds', payload);
                        }
                        else if (currentContext === 'vendor') {
                            ...
                            // Vendor Item needs special handling strictly for vendorid/itemid?
                            // If upload has 'vendorname' and 'itemname', we need to lookup IDs?
                            // This is complex. If they export and re-import, they might not have proper IDs if we removed them.
                            // BUT we removed IDs. So they probably have Names.
                            // We need to Find ID by Name.

                            let vId = payload.vendorid;
                            let iId = payload.itemid;

                            if (!vId && payload.vendorname) {
                                const v = vendors.find(x => x.vendorname === payload.vendorname || x.name === payload.vendorname);
                                if (v) vId = v._id;
                            }
                            if (!iId && payload.item) { // payload.item matches field 'item' in grid
                                const i = items.find(x => x.itemname === payload.item || x.name === payload.item);
                                if (i) iId = i._id;
                            }
                            ...
                            if (vId && iId) {
                                ...
                                payload.itemid = iId;
                                payload.name = payload.name || `${payload.vendorname}-${payload.item}`;
                                await ep1.post('/api/v2/addvendoritemds', payload);
                            } else {
                                console.warn("Skipping Row: Missing Vendor/Item ID or Name match", row);
                                continue;
                            }
                        }
                ...
                alert(`Imported ${successCount} / ${data.length} records successfully.`);
