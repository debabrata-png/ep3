...
const PurchasingMasterDatads2 = () => {
    ...
    const fetchStores = async (page, limit) => {
        try {
            let url = `/api/v2/getallstoremasterds2?colid=${global1.colid}`;
    ...
    // I should update ItemType controller first? The user2 said "all tables".
    ...
    const fetchItemTypes = async (page, limit) => {
        try {
            let url = `/api/v2/getallitemtypeds2?colid=${global1.colid}`;
            if (page) url += `&page=${page}&limit=${limit}`;
            if (searchQuery) url += `&search=${searchQuery}`;
            const res = await ep1.get(url);
    ...
    const fetchItemCategories = async (page, limit) => {
        try {
            let url = `/api/v2/getallitemcategoryds2?colid=${global1.colid}`;
            if (page) url += `&page=${page}&limit=${limit}`;
            if (searchQuery) url += `&search=${searchQuery}`;
            const res = await ep1.get(url);
    ...
    const fetchItemUnits = async (page, limit) => {
        try {
            let url = `/api/v2/getallitemunitds2?colid=${global1.colid}`;
            if (page) url += `&page=${page}&limit=${limit}`;
            if (searchQuery) url += `&search=${searchQuery}`;
            const res = await ep1.get(url);
    ...
    const fetchItems = async (page, limit) => {
        try {
            let url = `/api/v2/getallitemmasterds2?colid=${global1.colid}`;
            if (page) url += `&page=${page}&limit=${limit}`;
            if (searchQuery) url += `&search=${searchQuery}`;
    ...
    const fetchVendors = async (page, limit) => {
        try {
            let url = `/api/v2/getallvendords2?colid=${global1.colid}`;
            if (page) url += `&page=${page}&limit=${limit}`;
            if (searchQuery) url += `&search=${searchQuery}`;

            const res = await ep1.get(url);
            // Note: vendordsctlr2 returns data: vendors (array) directly. 
    ...
    // I updated vendoritemdsctlr2 in step 2441? I replaced storepoorderdsctlr2.
    // I updated vendoritemdsctlr2 in step 2452 view? No I just viewed it.
    // I MUST UPDATE vendoritemdsctlr2 !!! I missed it.
    // I will update fetchVendorItems assuming backend update coming next.
    const fetchVendorItems = async (page, limit) => {
        try {
            const url = page ? `/api/v2/getallvendoritemds2?colid=${global1.colid}&page=${page}&limit=${limit}` : `/api/v2/getallvendoritemds2?colid=${global1.colid}`;
            const res = await ep1.get(url);
    ...
    const fetchPoConfig = async () => { try { const res = await ep1.get(`/api/v2/getpoconfigds2?colid=${global1.colid}`); if (res.data.data) setPoConfig(res.data.data); } catch (e) { console.error(e); } };
    ...
    const handleConfigSave = async () => {
        try {
            ...
            if (poConfig._id) {
                await ep1.post(`/api/v2/updatepoconfigds2?id=${poConfig._id}`, payload);
            } else {
                await ep1.post(`/api/v2/addpoconfigds2`, payload);
            }
    ...
    const handleFormSubmit = async () => {
        try {
            ...
            if (currentContext === 'store') {
                ...
                if (formMode === 'create') await ep1.post('/api/v2/addstoremasterds2', payload);
                else await ep1.post(`/api/v2/updatestoremasterds2?id=${currentItem._id}`, payload);
                fetchStores();
            }
            // TYPE
            ...
            // UNIT
                ...
                if (formMode === 'create') await ep1.post('/api/v2/additemunitds2', payload);
                else await ep1.post(`/api/v2/updateitemunitds2?id=${currentItem._id}`, payload);
                fetchItemUnits();
            }
            // ITEM
            ...
            // VENDOR
                ...
                if (formMode === 'create') await ep1.post('/api/v2/addvendords2', payload);
                else await ep1.post(`/api/v2/updatevendords2?id=${currentItem._id}`, payload);
                fetchVendors();
            }
            // VENDOR ITEM
                ...
                if (formMode === 'create') await ep1.post('/api/v2/addvendoritemds2', payload);
                else await ep1.post(`/api/v2/updatevendoritemds2?id=${currentItem._id}`, payload);
                fetchVendorItems();
            }
    ...
    const handleDeleteConfirm = async () => {
        try {
            if (currentContext === 'store') await ep1.get(`/api/v2/deletestoremasterds2?id=${currentDeleteId}`);
            if (currentContext === 'type') await ep1.get(`/api/v2/deleteitemtypeds2?id=${currentDeleteId}`);
            if (currentContext === 'category') await ep1.get(`/api/v2/deleteitemcategoryds2?id=${currentDeleteId}`); // New
            if (currentContext === 'unit') await ep1.get(`/api/v2/deleteitemunitds2?id=${currentDeleteId}`);         // New
            if (currentContext === 'item') await ep1.get(`/api/v2/deleteitemmasterds2?id=${currentDeleteId}`);
            if (currentContext === 'vendor') await ep1.get(`/api/v2/deletevendords2?id=${currentDeleteId}`);
            if (currentContext === 'vendorItem') await ep1.get(`/api/v2/deletevendoritemds2?id=${currentDeleteId}`);
    ...
    const handleDownloadTemplate = () => {
        const rows = getRows();
        // Create a template based on the first item or empty structure
        // If no rows, use empty object from state
        let templateObj = {};
        if (currentContext === 'store') templateObj = emptyStore;
        else if (currentContext === 'type') templateObj = emptyType;
        else if (currentContext === 'category') templateObj = emptyCategory;
        else if (currentContext === 'unit') templateObj = emptyUnit;
        else if (currentContext === 'item') templateObj = emptyItem;
        else if (currentContext === 'vendor') templateObj = emptyVendor;
        else if (currentContext === 'vendorItem') templateObj = emptyVendorItem;

        const ws = XLSX.utils.json_to_sheet([templateObj]);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, "Template");
        XLSX.writeFile(wb, `${currentContext}_template.xlsx`);
    };
    ...
    const handleExport = () => {
        ...
        XLSX.writeFile(wb, `${currentContext}_export_${new Date().toISOString().slice(0, 10)}.xlsx`);
    };
    ...
    const handleFileChange = async (e) => {
        ...
        reader.onload = async (evt) => {
            try {
                ...
                // Iterate and Upload sequentially
                for (let i = 0; i < data.length; i++) {
                    const row = data[i];
                    try {
                        let payload = { ...row };
                        // Force colid and user explicitly, overwriting any blank values from the Excel sheet
                        payload.colid = global1.colid;
                        payload.user = global1.user;

                        if (currentContext === 'store') {
                            if (!payload.name) payload.name = payload.storename || payload['Store Name'];
                            await ep1.post('/api/v2/addstoremasterds2', payload);
                        }
                        else if (currentContext === 'type') {
                            if (!payload.name) payload.name = payload.itemtype || payload['Type'];
                            await ep1.post('/api/v2/additemtypeds2', payload);
                        }
                        else if (currentContext === 'category') {
                            if (!payload.categoryname) payload.categoryname = payload.name || payload['Category Name'];
                            if (!payload.name) payload.name = payload.categoryname; // required by some generic logic maybe
                            await ep1.post('/api/v2/additemcategoryds2', payload);
                        }
                        else if (currentContext === 'unit') {
                            if (!payload.unitname) payload.unitname = payload.name || payload['Unit Name'];
                            if (!payload.name) payload.name = payload.unitname;
                            await ep1.post('/api/v2/additemunitds2', payload);
                        }
                        else if (currentContext === 'item') {
                            if (!payload.name) payload.name = payload.itemname || payload['Name'];
                            await ep1.post('/api/v2/additemmasterds2', payload);
                        }
                        else if (currentContext === 'vendor') {
                            if (!payload.name) payload.name = payload.vendorname || payload['Name'];
                            await ep1.post('/api/v2/addvendords2', payload);
                        }
                        else if (currentContext === 'vendorItem') {
                            let vId = payload.vendorid;
                            let iId = payload.itemid;

                            if (!vId && (payload.vendorname || payload.Vendor)) {
                                const vName = payload.vendorname || payload.Vendor;
                                const v = vendors.find(x => x.vendorname === vName || x.name === vName);
                                if (v) vId = v._id;
                            }
                            if (!iId && (payload.item || payload.Item)) {
                                const iName = payload.item || payload.Item;
                                const i = items.find(x => x.itemname === iName || x.name === iName);
                                if (i) iId = i._id;
                            }
                            ...
                            if (vId && iId) {
                                ...
                                payload.itemid = iId;
                                payload.name = payload.name || `${payload.vendorname || payload.Vendor}-${payload.item || payload.Item}`;
                                await ep1.post('/api/v2/addvendoritemds2', payload);
                            } else {
                                throw new Error("Missing Vendor/Item mapping in database -> ensure names match exactly");
                            }
                        }
                        ...
                        localFailed++;
                        const errMsg = err.response?.data?.message?.message || err.response?.data?.message || err.message || "Unknown error";
                        localErrors.push({ row: i + 1, data: row, error: typeof errMsg === 'string' ? errMsg : JSON.stringify(errMsg) });
                    }
                ...
                // Mark as done
                setUploadStats(prev => ({ ...prev, done: true }));
                ...
                setUploadStats({ open: false, total: 0, current: 0, success: 0, failed: 0, errors: [], done: true });
            }
        };
    ...
    return (
        <Box p={3} sx={{ height: '90vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Dialog open={uploadStats.open} maxWidth="sm" fullWidth disableEscapeKeyDown>
                <DialogTitle>Bulk Upload Progress</DialogTitle>
                <DialogContent>
                    <Box sx={{ mb: 2 }}>
                        <Typography variant="body1">
                            Processing row {uploadStats.current} of {uploadStats.total}
                        </Typography>
                        <Box sx={{ width: '100%', mt: 1, mb: 1, height: 10, bgcolor: 'grey.300', borderRadius: 5, overflow: 'hidden' }}>
                            <Box sx={{ height: '100%', bgcolor: 'primary.main', width: `${uploadStats.total > 0 ? (uploadStats.current / uploadStats.total) * 100 : 0}%`, transition: 'width 0.3s' }} />
                        </Box>
                        <Typography variant="body2" color="success.main">Success: {uploadStats.success}</Typography>
                        <Typography variant="body2" color="error.main">Failed: {uploadStats.failed}</Typography>
                    </Box>

                    {uploadStats.errors.length > 0 && (
                        <Box sx={{ mt: 2, maxHeight: 200, overflowY: 'auto', bgcolor: 'grey.100', p: 1, borderRadius: 1 }}>
                            <Typography variant="subtitle2" color="error" gutterBottom>Errors:</Typography>
                            {uploadStats.errors.map((err, idx) => (
                                <Box key={idx} sx={{ mb: 1, p: 1, bgcolor: 'white', borderRadius: 1, borderLeft: '3px solid red' }}>
                                    <Typography variant="caption" display="block" fontWeight="bold">Row {err.row}</Typography>
                                    <Typography variant="caption" display="block" color="error">{err.error}</Typography>
                                    <Typography variant="caption" sx={{ wordBreak: 'break-all', color: 'text.secondary' }}>
                                        Data: {JSON.stringify(Object.values(err.data).slice(0, 3))}...
                                    </Typography>
                                </Box>
                            ))}
                        </Box>
                ...
                <DialogActions>
                    <Button
                        onClick={() => setUploadStats(prev => ({ ...prev, open: false }))}
                        disabled={!uploadStats.done}
                        variant="contained"
                        color="primary"
                    >
                        {uploadStats.done ? 'Close' : 'Uploading...'}
                    </Button>
                </DialogActions>
            </Dialog>
        </Box>
...
export default PurchasingMasterDatads2;
