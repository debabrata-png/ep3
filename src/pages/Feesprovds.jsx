...
const Feesprovds = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await ep1.post("/api/v2/getfeesprovds", { colid: global1.colid });
      if (res.data.success) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      let res;
      if (editMode) {
        res = await ep1.post("/api/v2/updatefeesprovds", { ...payload, id: formData._id });
      } else {
        res = await ep1.post("/api/v2/createfeesprovds", payload);
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await ep1.post("/api/v2/deletefeesprovds", { id, colid: global1.colid });
      if (res.data.success) {
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (evt) => {
      try {
        ...
        // Map global1 values to each row
        ...
        const res = await ep1.post("/api/v2/bulkfeesprovds", {
          data: mappedData,
          colid: global1.colid,
          user: global1.user
        });
        ...
        >
          ...
          <Container maxWidth="100%" sx={{ mt: 4, mb: 4 }}>
            <Box display="flex" marginBottom={4} marginTop={2}>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  XLSX.writeFile(wb, "provisional_fees.xlsx");
                }}
              >
                Export
              </Button>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  window.open(`${baseUrl}/api/v2/templatefeesprovds`, '_blank');
                }}
              >
                Template
              </Button>
            ...
            <Dialog open={openDialog} onClose={() => setOpenDialog(false)} maxWidth="md" fullWidth>
              <DialogTitle>{editMode ? "Edit Provisional Fee" : "Add Provisional Fee"}</DialogTitle>
              <DialogContent>
