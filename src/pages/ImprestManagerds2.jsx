...
const ImprestManagerds2 = () => {
    ...
    const fetchImprests = async () => {
        setLoading(true);
        try {
            const res = await ep1.get('/api/v2/getallpimprestds2', {
                params: {
                    colid: global1.colid,
                    page: page + 1,
                    limit: pageSize
    ...
    const handleOpen = async () => {
        ...
        try {
            ...
            // Fetch latest record to determine sequence
            const res = await ep1.get('/api/v2/getallpimprestds2', {
                params: {
                    colid: global1.colid,
                    page: 1,
            ...
            let newSequence = "001";
            if (res.data.success && res.data.data.imprests && res.data.data.imprests.length > 0) {
                const lastImprest = res.data.data.imprests[0];
                if (lastImprest.imprestcode && lastImprest.imprestcode.startsWith(prefix)) {
            ...
            newImprestCode = `${prefix}${newSequence}`;

        } catch (error) {
            console.error("Error generating imprest code:", error);
            // Fallback if fetch fails
            newImprestCode = `IMP-${Date.now()}`;
        }

        setFormData({
            imprestcode: newImprestCode,
            amount: '',
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await ep1.get(`/api/v2/deletepimprestds2?id=${id}`);
            fetchImprests();
        } catch (error) {
            console.error(error);
    ...
    const handleStatusUpdate = async (id, newStatus) => {
        ...
        try {
            await ep1.post(`/api/v2/updatepimprestds2?id=${id}`, {
                status: newStatus,
                colid: global1.colid
            });
            fetchImprests();
    ...
    const handleSubmit = async () => {
        ...
        try {
            if (isEdit) {
                await ep1.post(`/api/v2/updatepimprestds2?id=${currentId}`, payload);
            } else {
                await ep1.post(`/api/v2/addpimprestds2`, payload);
            }
            setOpenDialog(false);
            fetchImprests();
        } catch (error) {
    ...
    const activeNumToWords = (price) => {
        ...
        let str = "";
        let digit = Array.from(String(price), Number);
        let len = digit.length;

        const handleTens = (d1, d2) => {
            if (d1 === 1) return dblDigit[d2];
            else return tensPlace[d1] + (d2 !== 0 ? " " + sglDigit[d2] : "");
        };

        if (price === 0) return "Zero Rupees Only";

        if (len >= 8) { // Crores
            let croreVal = Math.floor(price / 10000000);
            if (croreVal > 0) str += activeNumToWords(croreVal).replace(" Rupees Only", "") + " Crore ";
            price = price % 10000000;
            digit = Array.from(String(price), Number);
            len = digit.length;
        }

        if (len >= 6) { // Lakhs
            let lakhVal = Math.floor(price / 100000);
            if (lakhVal > 0) str += activeNumToWords(lakhVal).replace(" Rupees Only", "") + " Lakh ";
            price = price % 100000;
            digit = Array.from(String(price), Number);
            len = digit.length;
        }
        ...
        if (len >= 4) { // Thousands
            let thousandVal = Math.floor(price / 1000);
            if (thousandVal > 0) str += activeNumToWords(thousandVal).replace(" Rupees Only", "") + " Thousand ";
            price = price % 1000;
            digit = Array.from(String(price), Number);
            len = digit.length;
        }
        ...
        if (len === 2) {
            str += handleTens(digit[0], digit[1]);
        } else if (len === 1 && price !== 0) {
            str += sglDigit[digit[0]];
        }
...
export default ImprestManagerds2;
