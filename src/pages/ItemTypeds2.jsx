...
const ItemTypeds2 = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            if (editId) await ep1.post(`/api/v2/updateitemtypeds2?id=${editId}`, payload);
            else await ep1.post('/api/v2/additemtypeds2', payload);
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Delete?')) {
            await ep1.delete(`/api/v2/deleteitemtypeds2?id=${id}`);
            fetchData();
        }
    };
