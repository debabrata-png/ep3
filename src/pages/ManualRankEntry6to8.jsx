...
const StudentMarksheetViewPage6to8ds = () => {
    ...
    const fetchInitialData = async (cid) => {
        try {
            ...
            setAvailableSemesters(['Class 6', 'Class 7', 'Class 8']);
    ...
    const generateMarksheetPDF = async (pdfData) => {
        ...
        // Vertical Headers
        // MODIFIED FOR CLASS 6-8: "Pre Mid Term" instead of "Periodic Test"
        const vHeaders = [
        ...
        let dy = my + h3;
        const dRH = 25;
        const allScholasticSubjects = pdfData.subjects.filter(sub => (!sub.isAdditional || sub.isAdditional === 'false') && sub.subjectname !== 'Teacher Remarks');

        let hasFailure = false;
        allScholasticSubjects.forEach(sub => {
            const isE = sub.term2Grade && sub.term2Grade.toUpperCase() === 'E';
            if (isE) hasFailure = true;
        });

        // DO NOT filter out failed subjects from the Part I display
        const scholasticSubjects = allScholasticSubjects.filter(sub => {
            const hasMarks = [
        ...
        scholasticSubjects.forEach(sub => {
            ...
            const t1TotDisplay = isabsent ? 'AB' : Number(t1Tot.toFixed(1));

            // Check E Grade was done before filtering

            gT1Obt += t1Tot;
            gT2Obt += parseFloat(t2Tot); // Use parseFloat here as t2Tot
        ...
        const maxPossPerTerm = scholasticSubjects.length * 100; // Calculate max on passed subjects only
        ...
        // --- PAGE 3: CO-SCHOLASTIC & ADDITIONAL & COMPARTMENT ---
        doc.addPage();
        drawRect(10, 10, 575, 822, { lineWidth: 4, strokeColor: mainBorderColor });
        drawRect(20, 20, 555, 802, { lineWidth: 1.5 });
        ...
        // --- COMPARTMENT TABLE (For Class 6-8) ---
        ...
        if (true) {
            ...
            // Headers: SR. NO. | SUBJECT | MAX MARKS | MARKS OBTAINED | RESULT
            const compHeaders = ["SR. NO.", "SUBJECT", "MAX MARKS", "MARKS OBTAINED", "RESULT"];
            const compWs = [cW * 0.1, cW * 0.35, cW * 0.15, cW * 0.2, cW * 0.2]; // widened MARKS OBTAINED from 0.15 to 0.2
    ...
    return (
        <Box sx={{ p: 3 }}>
            <Typography variant="h4" gutterBottom>
                Student Marksheet View (Class 6 - 8)
            </Typography>
                            ...
                            <Table size="small">
                                <TableHead sx={{ bgcolor: '#f5f5f5' }}>
                                    <TableRow>
                                        ...
                                        <TableCell align="center"><strong>T1 Pre Mid</strong></TableCell>
                                        <TableCell align="center"><strong>T1 Mid Exam</strong></TableCell>
                                        <TableCell align="center"><strong>T1 Grade</strong></TableCell>
                                        <TableCell align="center"><strong>T2 Pre Mid</strong></TableCell>
                                        <TableCell align="center"><strong>T2 Annual</strong></TableCell>
                                        <TableCell align="center"><strong>T2 Grade</strong></TableCell>
                                    </TableRow>
                                </TableHead>
                                <TableBody>
                                    {marks[0].subjects && marks[0].subjects.filter(s => (!s.isAdditional || s.isAdditional === 'false') && s.subjectname !== 'Teacher Remarks').map((sub, index) => {
                                        const isGrace = sub.isgrace || false;
                                        return (
                                            <TableRow key={index}>
            ...
            {/* PDF Generation Dialog to collect extra info */}
                ...
                <DialogContent>
                    <Box sx={{ pt: 1 }}>
                        ...
                        <TextField
                            ...
                            fullWidth
                            InputLabelProps={{ shrink: true }}
                            value={pdfParams.manualRank}
                            onChange={(e) => setPdfParams({ ...pdfParams, manualRank: e.target.value })}
                        />
                    </Box>
                </DialogContent>
                <DialogActions>
...
export default StudentMarksheetViewPage6to8ds;
