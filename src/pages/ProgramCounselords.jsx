...
const ProgramCounselords = () => {
    ...
    const handleDownloadTemplate = () => {
        const templateData = [
            {
                ...
                "Counselor Name": "John Doe",
                "Counselor Email": "john@example.com",
                "Duration": "4 Years",
                "Eligibility": "12th Pass",
                "Total Fee": 100000,
                "Application Fee": 1000,
                "First Installment": 50000,
                "Installments": 2,
                "Brochure URL": "",
                "Syllabus URL": "",
        ...
        XLSX.writeFile(wb, "program_counselor_template.xlsx");
    };
        ...
        for (const row of data) {
            try {
                const payload = {
                    ...
                    brochure_url: row["Brochure URL"],
                    syllabus_url: row["Syllabus URL"],
                    placement_highlights: row["Placement Highlights"],
                    faculty_info: row["Faculty Info"],
                    accreditation: row["Accreditation"],
                    colid: global1.colid,
                    created_by: global1.user,
                };

                if (!payload.category || !payload.course_name) {
                    errorCount++;
                    continue;
                }

                await ep1.post("/api/v2/createprogramcounselords", payload);
                successCount++;
            } catch (err) {
    ...
    const handleOpenDialog = (program = null) => {
        if (program) {
            ...
            // Pre-populate user options if editing with existing counselor
            if (program.counsellor_name) {
                setUserOptions([{ name: program.counsellor_name, email: program.counsellor_email }]);
            }
        } else {
            setEditMode(false);
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (editMode) {
                await ep1.post("/api/v2/updateprogramcounselords", payload, {
        ...
        <Container maxWidth="xl" sx={{ mt: 6, mb: 6 }}>
            <Box sx={{ mb: 4, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                <Box sx={{ display: "flex", alignItems: "center" }}>
                    ...
                    <Typography variant="h4" sx={{ fontWeight: 700, color: "#1e293b" }}>Program Counselor Master</Typography>
                </Box>
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="md" fullWidth>
                ...
                <DialogContent>
                    <Box sx={{ display: "flex", flexDirection: "column", gap: 2, mt: 2 }}>
                        ...
                        {/* Replaced TextFields with Autocomplete for User Search */}
                        <Autocomplete
                            options={userOptions}
                            getOptionLabel={(option) => {
                                // Handle both full object options and when val is just a string (though we control it)
                                if (typeof option === 'string') return option;
                                return `${option.name} (${option.email})`;
                            }}
                            loading={loadingUsers}
                            value={formData.counsellor_email ? { name: formData.counsellor_name, email: formData.counsellor_email } : null}
                            onChange={(event, newValue) => {
                                if (newValue) {
                                    setFormData({
                                        ...formData,
                                        counsellor_name: newValue.name,
                                        counsellor_email: newValue.email
                                    });
                                } else {
                                    setFormData({
                                        ...formData,
...
export default ProgramCounselords;
