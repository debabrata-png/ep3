...
const Vendorproductmanagementds2 = () => {
  ...
  const fetchVendorProducts = async () => {
    setLoading(true);
    try {
      const response = await ep1.get(`/api/v2/getallvendorproductds2?colid=${global1.colid}`);
  ...
  const fetchProducts = async () => {
    try {
      const response = await ep1.get(`/api/v2/getallproductds2?colid=${global1.colid}`);
  ...
  const handleAdd = async () => {
    try {
      ...
      await ep1.post("/api/v2/addvendorproductds2", payload);
  ...
  const handleUpdate = async () => {
    try {
      ...
      await ep1.post(`/api/v2/updatevendorproductds2?id=${editId}`, payload);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this vendor product?")) {
      try {
        await ep1.get(`/api/v2/deletevendorproductds2?id=${id}`);
  ...
  const handleBulkUpload = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        await ep1.post("/api/v2/bulkaddvendorproductds2", { vendorproducts: vendorProductsArray });
...
export default Vendorproductmanagementds2;
