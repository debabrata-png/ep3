...
const ManualComp11to12 = () => {
    ...
    const [pdfParams, setPdfParams] = useState({
        ...
        newSessionDate: '',
        manualRank: '',
        subjectOverrides: {} // { subjectcode: grade }
    });
            ...
            if (response.data.success) {
                ...
                if (data.subjects) {
                    data.subjects.forEach(sub => {
                        overrides[sub.subjectcode] = sub.grade || '';
                    });
                }
    ...
    const handleGenerateClick = async () => {
        ...
        setTimeout(async () => {
            const finalData = {
                ...
                manualRank: pdfParams.manualRank,
                subjectOverrides: pdfParams.subjectOverrides
            };
    ...
    const createPDF = async (data) => {
        ...
        mainSubjects.forEach((sub, idx) => {
            ...
            const subGrade = (data.subjectOverrides && data.subjectOverrides[sub.subjectcode]) || sub.grade || '-';
            drawCenteredText(subGrade, xGrd, dy, wGrd, drH, 9, true);
        ...
        const rankToUse = data.manualRank || data.rank || '-';
        drawCenteredText(String(rankToUse), xUnit, dy, sTableW - (wSr + wSub), drH, 10, true);
        dy += drH;
        ...
        // Compartment Table — dynamically calculated from overridden grades
        const failedSubjectsMap = new Map();

        subjectsWithTotal.forEach(sub => {
            const finalGrade = (data.subjectOverrides && data.subjectOverrides[sub.subjectcode]) || sub.grade || '';
            const isFail = finalGrade.toUpperCase() === 'E' || finalGrade.toUpperCase() === 'FAIL';
            if (isFail) {
                failedSubjectsMap.set(sub.subjectcode, sub);
            }
        });

        // Also include any subjects that were already identified as compartment in backend (if any left out)
        if (data.compartmentSubjects) {
            data.compartmentSubjects.forEach(sub => {
                const subCode = sub.subjectcode || (typeof sub === 'string' ? sub : null);
                if (subCode && !failedSubjectsMap.has(subCode)) {
                    failedSubjectsMap.set(subCode, sub);
                }
            });
        }

        const failedSubjects = Array.from(failedSubjectsMap.values());
            ...
            <Dialog open={openDialog} onClose={() => setOpenDialog(false)}>
                ...
                <DialogContent>
                    <Box sx={{ pt: 2, display: 'flex', flexDirection: 'column', gap: 2 }}>
                        ...
                        <Typography variant="h6" sx={{ mt: 2 }}>Subject Grade Overrides</Typography>
                        <Grid container spacing={2}>
                            {fullPdfData?.subjects?.filter(sub => !sub.isAdditional && sub.subjectname !== 'Teacher Remarks').map((sub) => (
                                <Grid item xs={12} sm={6} key={sub.subjectcode}>
                                    <TextField
                                        fullWidth
                                        label={sub.subjectname}
                                        value={pdfParams.subjectOverrides[sub.subjectcode] || ''}
                                        onChange={(e) => setPdfParams({
                                            ...pdfParams,
                                            subjectOverrides: {
                                                ...pdfParams.subjectOverrides,
                                                [sub.subjectcode]: e.target.value
                                            }
                                        })}
                                        helperText="Enter 'E' or 'Fail' for Compartment"
                                    />
                                </Grid>
                            ))}
                        </Grid>
...
export default ManualComp11to12;
