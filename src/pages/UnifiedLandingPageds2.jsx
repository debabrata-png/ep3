...
const UnifiedLandingPageds1 = () => {
    ...
    const fetchLandingPages = async () => {
        try {
            const res = await ep1.get("/api/v2/getallunifiedlandingpagesds", {
    ...
    const generateUrl = (slug, source = null) => {
        ...
        let url = `${currentUrl}/publicunifiedlandingpageds2/${slug}?data=${encryptedData}`;
        if (source) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (editMode) {
                await ep1.post("/api/v2/updateunifiedlandingpageds", payload, {
                    params: { id: currentPage._id },
                });
                showSnackbar("Unified Landing page updated successfully", "success");
            } else {
                await ep1.post("/api/v2/createunifiedlandingpageds", payload);
                showSnackbar("Unified Landing page created successfully", "success");
            }

            fetchLandingPages();
            handleCloseDialog();
        } catch (err) {
            console.error("Error saving unified landing page:", err);
            showSnackbar("Failed to save unified landing page", "error");
        }
    };

    const handleDelete = async (id) => {
        if (window.confirm("Are you sure you want to delete this unified landing page?")) {
            try {
                await ep1.get(`/api/v2/deleteunifiedlandingpageds/${id}`);
                showSnackbar("Unified Landing page deleted successfully", "success");
                fetchLandingPages();
            } catch (err) {
                console.error("Error deleting unified landing page:", err);
    ...
    const handleSaveQr = async () => {
        ...
        try {
            await ep1.post("/api/v2/addunifiedqrcodeds", {
    ...
    const handleDeleteQr = async (qrId) => {
        if (window.confirm("Are you sure you want to delete this QR code?")) {
            try {
                await ep1.get("/api/v2/deleteunifiedqrcodeds", {
    ...
    return (
        <Container maxWidth="xl" sx={{ mt: 6, mb: 6 }}>
                            ...
                            <CardContent sx={{ flexGrow: 1, p: 3 }}>
                                ...
                                <Box sx={{ display: "flex", flexDirection: "column", gap: 1, mt: 2 }}>
                                    <Typography variant="body2" sx={{ color: "#64748b" }}>
                                        ...
                                        {/* Unified Landing Page - No Category Pre-defined */}
                                    </Typography>
                                    <Typography variant="body2" sx={{ color: "#64748b" }}>
            ...
            <Dialog open={openDialog} onClose={handleCloseDialog} maxWidth="md" fullWidth>
                <DialogTitle>{editMode ? "Edit Unified Landing Page" : "Create Unified Landing Page"}</DialogTitle>
                <DialogContent>
                    <Box sx={{ mt: 2 }}>
                        <Grid container spacing={2}>
                            ...
                            {/* Category Selection Removed for Unified Landing Page */}
                            {/* <Grid item xs={12}>
                <TextField
                  fullWidth
                  select
